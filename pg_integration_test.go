...
// TestPG_BasicCRUD 验证 PG 方言下基本 CRUD（镜像 TestMySQL_BasicCRUD）
...
// TestPG_WhereConditions 验证各类 WHERE 条件在 PG 方言下双引号转义正确（镜像 TestMySQL_WhereConditions）
func TestPG_WhereConditions(t *testing.T) {
	repo, _ := setupPGDB(t)
	ctx := context.Background()

	// 注：Like 用前缀匹配避开 PG 大小写敏感差异（与 advanced_complex_test 同策略）
	seeds := []MySQLUser{
	...
	t.Run("LikeRight_Prefix", func(t *testing.T) {
		// 用前缀匹配（LIKE 'Alp%'）避开 PG case-sensitive 差异
		q, u := NewQuery[MySQLUser](ctx)
	...
	t.Run("IsNull", func(t *testing.T) {
		// PG 中空字符串与 NULL 严格区分；本测试仅验证 IsNull 算子可执行不报错
		q, u := NewQuery[MySQLUser](ctx)
		q.IsNull(&u.Email)
		_, err := repo.List(q)
...
// TestPG_OrderGroupHaving 验证 ORDER BY / GROUP BY / HAVING 在 PG 方言下正确（镜像 TestMySQL_OrderGroupHaving）
func TestPG_OrderGroupHaving(t *testing.T) {
	repo, _ := setupPGDB(t)
	ctx := context.Background()
	...
	t.Run("GroupBy_Having_RawScan", func(t *testing.T) {
		// 注：HAVING 用 COUNT(*) 而非别名（PG 严格 SQL 不支持别名引用，与 repo_extra_test 同策略）
		type row struct {
...
// TestPG_JoinQuery 验证 LEFT JOIN ON 条件中双引号转义（镜像 TestMySQL_JoinQuery）
func TestPG_JoinQuery(t *testing.T) {
	repo, _ := setupPGDB(t)
	ctx := context.Background()
...
func TestPG_QuoteColumn(t *testing.T) {
	...
	t.Run("getQuoteChar_返回双引号", func(t *testing.T) {
		...
		assertEqual(t, `"`, qL, "PG qL 应为双引号")
		assertEqual(t, `"`, qR, "PG qR 应为双引号")
	})
	...
	t.Run("quoteColumn_PG方言", func(t *testing.T) {
		qL, qR := getQuoteChar(db)
		for _, c := range cases {
