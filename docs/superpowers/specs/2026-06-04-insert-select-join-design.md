...
- **输入**：`setupTestDB[ClosureSD]`，种子 4 行：`{anc=1,desc=5,depth=0}`（ext 正常候选，未删）、`{anc=5,desc=7,depth=0}`（sub 正常候选，未删）、`{anc=5,desc=88,depth=0}`（sub 干扰，Save 后 `db.Delete(&row)` 软删）、`{anc=2,desc=5,depth=0}`（ext 干扰，Save 后 `db.Delete(&row)` 软删）。源 query：`q.Unscoped().CrossJoinAs(sub).WhereRaw("sub.ancestor_id = ?", 5).Eq(&ext.DescendantID, 5).WhereRaw("ext.deleted_at IS NULL").WhereRaw("sub.deleted_at IS NULL")` + 3 个同 AC-1 的 `SelectRaw`。`InsertSelect(repo, ctx, []any{"ancestor_id","descendant_id","depth"}, q)`。
- **验证机制（两侧前缀均被真实验证，无死代码）**：ext 候选（`descendant_id=5`）= 未删 `1→5` + 已删 `2→5`；sub 候选（`ancestor_id=5`）= 未删 `5→7` + 已删 `5→88`。Unscoped 下两侧都不过滤会产生 4 行笛卡尔积 `{1,7,1} {1,88,1} {2,7,1} {2,88,1}`（affected=4）。**删掉 `sub.deleted_at IS NULL` 会多出 `{1,88,1}`（affected=2，sub 前缀被验证红）；删掉 `ext.deleted_at IS NULL` 会多出 `{2,7,1}`（affected=2，ext 前缀被验证红）**。两侧前缀都加 → 仅 `{1,7,1}`（affected=1）。两侧各有已删干扰行作区分点，任一前缀都不是死代码。
- **可观测输出**：返回 `(1, nil)`；新增行恰为 `{anc=1, desc=7, depth=1}`；新增未删行中既查无 `descendant_id=88`（sub 已删未复制）也查无 `ancestor_id=2`（ext 已删未复制）：`SELECT count(*) FROM closure_sd WHERE deleted_at IS NULL AND (descendant_id=88 OR ancestor_id=2)` = 0。
...
- **自插入自连接形态**：`INSERT INTO closure (...) SELECT ... FROM closure AS ext JOIN closure AS sub`（目标表 = 源表）。CI 仅 SQLite，三库真机未验证，但据官方文档可给出较确定结论：
  - **MySQL：允许（非 error 1093），自连接形态官方明确支持**。官方手册明确 INSERT...SELECT 的目标表**可出现在 SELECT 的顶层 FROM**（MySQL 内部建临时表中转），且**专门针对自连接给出指导**：「当 SELECT 与 INSERT 引用同一张表时，为 SELECT 中每个表用唯一别名并以别名限定列名」——正是本场景 `FROM closure AS ext JOIN closure AS sub` 形态，官方预期并支持（非过度乐观）。error 1093「can't specify target table ... in FROM」**仅限 UPDATE/DELETE 的同表子查询**，不适用 INSERT...SELECT 顶层 FROM。gplus 的 `InsertSelectTx` 用裸 `?` 内联子查询**无外层括号**（Round 2 实测 + 本轮 P1 SQL 印证 + GORM 源码 `statement.go AddVar` 的 `*gorm.DB` 分支 `writer.WriteString` 不加括号、跨方言一致），正是顶层 FROM 形态，落在允许侧 → **MySQL 风险实际为零**。来源：MySQL 8.4 手册 15.2.7.1（https://dev.mysql.com/doc/refman/8.4/en/insert-select.html）、Bug #6980（https://bugs.mysql.com/bug.php?id=6980）、GORM statement.go AddVar。
  - **PostgreSQL：允许，无限制**。INSERT 源 SELECT 复用标准 SELECT 语法，自连接别名唯一即可。来源：https://www.postgresql.org/docs/current/sql-insert.html。
  - **达梦 DM：语法层面有利，但有连接列约束 + 引号大小写两处真机项**。DM 官方「同一表的不同别名视为不同对象」（利于自连接）。**硬限制（必须满足）**：DM 官方规定「带连接的查询，连接中视图基表以外的表，连接列上必须是主键或带 UNIQUE 约束」——本场景连接列 `ancestor_id`/`descendant_id` 须在目标库有主键或唯一索引（closure 表通常 `(ancestor_id,descendant_id)` 联合主键/唯一索引可满足，但须确认），否则 DM 拒绝。另：裸前缀列解析依赖 `COMPATIBLE_MODE=2`，且 `CASE_SENSITIVE=Y` 下裸表名可能 UPPERCASE 后与双引号建表的小写真实表名不匹配。来源：https://eco.dameng.com/document/dm/zh-cn/pm/insertion-deletion-modification.html。
...
- **结论：方案 A 不退化**。MySQL/PG 官方既接受、DM 语法层面有利，故「纯验证 + 文档」不会退化成「文档说不支持」，无需回退方案 B/C。GORM 不给内联子查询加括号已由源码查证（`statement.go AddVar`），MySQL 不触发 1093。**残留真机项仅为 DM 两处**：① 连接列 `ancestor_id`/`descendant_id` 须有主键/UNIQUE 约束；② `CASE_SENSITIVE=Y` 下副别名裸表名引号大小写。

## 显式排除（Out of Scope）

- **InsertSelect 实现改动**：不改 `repository.go`（探针证明零改动可用）。
- **软删除自动前缀改写**：不做（YAGNI，拦 GORM callback 过度工程）。
- **DM 真机验证**：MySQL/PG 已由官方文档确认接受（见方言段），本轮 SQLite 为准；残留真机项仅 DM 两处（连接列主键/UNIQUE 约束 + 副别名引号大小写），待下游达梦库确认。
