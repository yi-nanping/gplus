...
- [ ] `find_as.go` 实现 4 个新函数 + `aggregateWrap[R]` 私有类型 + `aggregateAlias` 常量 + `ErrFindOneAsConflict` + godoc
- [ ] `repository.go:848` aggregate 改造（用 `aggregateWrap[R]` + alias `gplus_agg_v` + `.Find`）+ godoc 注释更新
- [ ] `find_as_test.go` 实现 `TestGORMCallbackBehaviorProbe`（6 条 GORM 行为锁定断言，§4.1）
- [ ] `find_as_test.go` 完成 callback 触发矩阵 + 投影 table-driven + 边界（含 `ErrFindOneAsConflict` 用例）
- [ ] `aggregate` 已有测试不退化（Sum/Max/Min/Avg 全绿，含空表 NULL）
- [ ] `D:/Environment/golang/go1.21.11/bin/go.exe test -race ./...` 全绿
- [ ] **新增代码独立覆盖率 ≥ 94%；整体覆盖率不低于 93.5%**（aggregateWrap 类型 + 4 个新函数加入分母后基线变化）
- [ ] `CHANGELOG.md` 增 `## [0.7.0]` 条目（含强措辞迁移建议 + 两条互补 grep + 性能基线 + GORM 版本锁定）
- [ ] `README.md` 同步新 API + 跨租户后果说明 + 排查命令 + DataRule JOIN 警告 + col 字符串警告
