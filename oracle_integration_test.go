...
func TestOracle_OrderGroupHaving(t *testing.T) {
	...
	t.Run("GroupBy_Having_RawScan", func(t *testing.T) {
		...
		// （Oracle 不带引号的列名默认输出 UPPERCASE，会与小写 struct tag 不匹配）
		type row struct {
...
func TestOracle_QuoteColumn(t *testing.T) {
	...
	t.Run("getQuoteChar_返回空_quoter", func(t *testing.T) {
		...
		assertEqual(t, "", qL, "Oracle qL 应为空字符串（避免 ORA-00904）")
		assertEqual(t, "", qR, "Oracle qR 应为空字符串")
	})

	// 空 quoter 下 quoteColumn 输出原样（让 Oracle 自身 UPPERCASE 解析裸标识符）
	cases := []struct {
		input string
		want  string
	}{
